...
public:
    ...
    OP_REQUIRES_OK(context, context->GetAttr("sampling_ratio", &sampling_ratio_));
    is_nhwc_ = false;
...
void Compute(tensorflow::OpKernelContext* context) override {
	...
	if (is_nhwc_) {
		std::vector<int64> shape = {RoIDim0, pooled_height_, pooled_width_, X.dim_size(3)};
		OP_REQUIRES_OK(context, TensorShapeUtils::MakeShape(shape, &output_shape));
	}
	else {
		std::vector<int64> shape = {RoIDim0, X.dim_size(1), pooled_height_, pooled_width_};
		OP_REQUIRES_OK(context, TensorShapeUtils::MakeShape(shape, &output_shape));
	}
...
void Compute(tensorflow::OpKernelContext* context) override {
	...
	const GPUDevice& d = context->eigen_device<GPUDevice>();
	typename TTypes<float, 4>::ConstTensor input_tensor(inputs.tensor<float, 4>());
	typename TTypes<float, 4>::ConstTensor input_grads(grads.tensor<float, 4>());
	typename TTypes<float, 2>::ConstTensor rois(RoIs.tensor<float, 2>());
